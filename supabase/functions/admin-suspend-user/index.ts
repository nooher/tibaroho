/* admin-suspend-user — admin indefinitely suspend a user account.
 *
...
export default async function handler(req: Request): Promise<Response> {
  ...
  try {
    ...
    // Indefinite ban — auth-admin updates banned_until to a far-future timestamp.
    const { error } = await admin.auth.admin.updateUserById(user_id, { ban_duration: '876000h' })
    if (error) {
      await audit(caller.id, String((profile as Record<string, unknown>).name || caller.id), 'user.suspend', user_id, { error: error.message }, 'error')
      return new Response(JSON.stringify({ error: error.message }), { status: 500, headers: CORS })
    }
    await audit(caller.id, String((profile as Record<string, unknown>).name || caller.id), 'user.suspend', user_id, {})
