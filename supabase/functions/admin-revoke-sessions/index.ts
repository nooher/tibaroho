/* admin-revoke-sessions — admin sign a user out of all sessions globally.
 *
...
export default async function handler(req: Request): Promise<Response> {
  ...
  try {
    ...
    if (error) {
      await audit(caller.id, String((profile as Record<string, unknown>).name || caller.id), 'user.signOutAll', user_id, { error: error.message }, 'error')
      return new Response(JSON.stringify({ error: error.message }), { status: 500, headers: CORS })
    }
    await audit(caller.id, String((profile as Record<string, unknown>).name || caller.id), 'user.signOutAll', user_id, {})
