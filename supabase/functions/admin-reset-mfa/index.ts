/* admin-reset-mfa — admin reset all MFA factors for a user.
 *
...
export default async function handler(req: Request): Promise<Response> {
  ...
  try {
    ...
    // List the user's MFA factors and delete each. After this, the user is
    // prompted to set up MFA fresh on next sign-in.
    const { data: factors, error: listErr } = await admin.auth.admin.mfa.listFactors({ userId: user_id })
    if (listErr) {
      await audit(caller.id, String((profile as Record<string, unknown>).name || caller.id), 'user.resetMfa', user_id, { error: listErr.message }, 'error')
    ...
    await audit(caller.id, String((profile as Record<string, unknown>).name || caller.id), 'user.resetMfa', user_id, { removed })
