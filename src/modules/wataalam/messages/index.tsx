...
export default function ProviderMessages() {
  ...
  return (
    <div style={{ padding: '24px 24px 80px', fontFamily: TYPE.sans }}>
      <h1 style={{ fontFamily: TYPE.serif, fontWeight: 800, color: JEWEL.tealDeep, fontSize: 32 }}>Ujumbe</h1>
      <p style={{ color: TEXT.muted, marginTop: 0 }}>Mazungumzo na wagonjwa wako, kikundi cha rika, na msimamizi.</p>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'minmax(220px, 280px) 1fr', gap: 18, marginTop: 18 }}>
        <aside style={{ display: 'grid', gap: 10, alignContent: 'start' }}>
          {channels.map((c) => (
            <button
              key={c.id}
              onClick={() => open(c.id)}
              aria-label={`Fungua ${c.title_sw ?? c.id}`}
              style={{
                textAlign: 'left',
                padding: 12,
                borderRadius: 14,
            ...
            >
              ...
              {c.unread > 0 && <div style={{ marginTop: 6 }}><span style={{ background: JEWEL.goldHope, color: '#0B0908', borderRadius: 999, padding: '2px 8px', fontSize: 11, fontWeight: 700 }}>{c.unread} mpya</span></div>}
            </button>
          ))}
        </aside>
        ...
        <section style={{ background: '#FAF5E5', border: `1px solid ${hexToRgba('#000', 0.08)}`, borderRadius: 18, padding: 14, minHeight: 480, display: 'flex', flexDirection: 'column' }}>
          {!openId ? (
            <p style={{ color: TEXT.muted, alignSelf: 'center', marginTop: 60 }}>Chagua mazungumzo upande wa kushoto.</p>
          ) : (
            <>
                  ...
                  return (
                    <div key={m.id} style={{ alignSelf: mine ? 'flex-end' : 'flex-start', maxWidth: '78%' }}>
                      ...
                      <div style={{ fontSize: 10, color: TEXT.muted, marginTop: 2, textAlign: mine ? 'right' : 'left' }}>
                        {new Date(m.ts).toLocaleTimeString('sw-TZ')} {mine && (seenByOther ? '· Imeonekana' : '· Imetumwa')}
                      </div>
                    </div>
                  )
                })}
              ...
              {typers.length > 0 && (
                <div style={{ fontSize: 12, color: TEXT.muted, padding: '4px 6px' }}>{typers.join(', ')} anaandika…</div>
              )}
              <div style={{ display: 'flex', gap: 8, marginTop: 8 }}>
                ...
                />
                <button onClick={send} style={{ padding: '8px 18px', borderRadius: 999, background: JEWEL.tealMwenza, color: '#FAF5E5', border: 'none', fontWeight: 700, cursor: 'pointer' }}>Tuma</button>
              </div>
            </>
          )}
        </section>
