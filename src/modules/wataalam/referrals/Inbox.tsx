...
export default function ReferralsInbox() {
  ...
  return (
    <div style={{ padding: '24px 22px 80px', fontFamily: TYPE.sans }}>
      <h1 style={{ fontFamily: TYPE.serif, fontWeight: 800, color: JEWEL.tealDeep, fontSize: 30 }}>{t('wataalam.refinbox.title', 'Rufaa zilizopokelewa')}</h1>
      <p style={{ color: TEXT.muted }}>{t('wataalam.refinbox.subtitle', 'Thibitisha umepokea, kisha sasisha hali ya kufunga mzunguko.')}</p>
      ...
      <div style={{ display: 'grid', gap: 12, marginTop: 18 }}>
        {items.map((r) => (
          <div key={r.id} style={{ padding: 14, background: '#FAF5E5', borderRadius: 14, border: `1px solid ${hexToRgba('#000', 0.08)}` }}>
            ...
            <div style={{ fontSize: 13, color: TEXT.muted }}>{t('wataalam.refinbox.from', 'Kutoka:')} {r.fromProvider} · {new Date(r.ts).toLocaleDateString('sw-TZ')}</div>
            <p style={{ marginTop: 8 }}>{r.reasonSw}</p>
            <div style={{ display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap' }}>
              {!r.acked && (
                <button onClick={() => ack(r.id)} style={{ padding: '6px 12px', borderRadius: 999, background: JEWEL.tealMwenza, color: '#FAF5E5', border: 'none', cursor: 'pointer', fontWeight: 600 }}>{t('wataalam.refinbox.confirm', 'Thibitisha')}</button>
              )}
              <label style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                <span style={{ fontSize: 12, color: TEXT.muted }}>{t('wataalam.refinbox.status', 'Hali:')}</span>
