...
export default function EmergencyPage() {
  ...
  return (
    <PageShell title={t('mimi.er.title', 'Dharura · msaada wa haraka')} subtitle={t('mimi.er.subtitle', 'Sio peke yako. Mwenza yuko nawe sasa.')} back={{ to: '/mimi' }} flagBar>
      {/* Red glass band */}
      <section
        style={{
        ...
        aria-live="assertive"
      >
        <Pill tone="maroon">{t('mimi.er.immediate', 'Hatua za mara moja')}</Pill>
        <h2 style={{ fontFamily: TYPE.serif, fontSize: 28, margin: '10px 0 6px', letterSpacing: TYPE.tighterTrack }}>{t('mimi.er.breathe', 'Pumua. Wewe ni salama hapa.')}</h2>
        <p style={{ margin: 0, color: TEXT.body, fontSize: 14 }}>
          {t('mimi.er.crisis-msg', 'Kama unawaza kujidhuru au mtu mwingine yuko hatarini, tafadhali piga simu sasa. Mwenza atakuwa mlinzi wako.')}
        </p>
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10, marginTop: 16 }}>
          <a href="tel:112" style={primaryBtn}>{t('mimi.er.call-112', '📞 Piga 112 — Dharura')}</a>
          <Link to="/rafiki?mode=mlinzi" style={secondaryBtn}>{t('mimi.er.guardian-mode', 'Mwenza · mlinzi mode →')}</Link>
        </div>
      </section>
      ...
      <section
        ...
        aria-label={t('mimi.er.hotlines-aria', 'Namba za msaada')}
      >
        <Pill tone="teal">{t('mimi.er.hotlines', 'Namba za msaada')}</Pill>
        <h3 style={{ fontFamily: TYPE.serif, fontSize: 22, margin: '10px 0 14px', letterSpacing: TYPE.tighterTrack }}>{t('mimi.er.call-free', 'Piga simu — bure')}</h3>
        {HOTLINES.map((h) => (
          <a
            key={h.number}
          ...
          >
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              {!h.verified && <Pill tone="gold" ariaLabel={t('mimi.er.unverified-aria', 'Hakijahakikishwa')}>{t('mimi.er.verify', 'thibitisha')}</Pill>}
        ...
        <p style={{ marginTop: 12, color: TEXT.muted, fontSize: 11 }}>
          {t('mimi.er.verify-note', 'Namba zilizo na "thibitisha" zinahitaji uthibitisho wa hivi karibuni kabla ya matumizi ya mwisho.')}
        </p>
      </section>
      ...
      {phase === 'hub' && (
        <section style={{ ...glass(JEWEL.indigoWisdom, 0.18), padding: 22, borderRadius: RADII.sheet, marginBottom: 20 }}>
          <Pill tone="indigo">{t('mimi.er.quick-check', 'Uchunguzi wa haraka')}</Pill>
          <h3 style={{ fontFamily: TYPE.serif, fontSize: 22, margin: '10px 0 6px', letterSpacing: TYPE.tighterTrack }}>{t('mimi.er.cssrs-short', 'C-SSRS kifupi')}</h3>
          <p style={{ color: TEXT.muted, fontSize: 14, margin: '0 0 14px' }}>
            {t('mimi.er.cssrs-intro', 'Maswali 6 mafupi yatasaidia kupima usalama wako sasa. Hakuna jibu sahihi au baya.')}
          </p>
          <button onClick={() => setPhase('cssrs')} style={primaryBtn}>{t('mimi.er.start-cssrs', 'Anza C-SSRS →')}</button>
        </section>
      )}
      ...
      {phase === 'result' && out && (
        <section style={{ ...glass(out.redFlag ? JEWEL.maroonCrisis : JEWEL.tealRoho, 0.3), padding: 22, borderRadius: RADII.sheet }}>
          ...
          <div style={{ display: 'flex', gap: 10, marginTop: 14, flexWrap: 'wrap' }}>
            {out.redFlag && <a href="tel:112" style={primaryBtn}>{t('mimi.er.call-112-now', '📞 Piga 112 sasa')}</a>}
            <Link to="/rafiki?mode=mlinzi" style={secondaryBtn}>{t('mimi.er.talk-rafiki', 'Zungumza na Rafiki')}</Link>
            <button onClick={() => setPhase('hub')} style={secondaryBtn}>{t('mimi.er.back', 'Rudi')}</button>
          </div>
        </section>
      )}
