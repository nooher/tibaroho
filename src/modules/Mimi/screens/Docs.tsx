...
export default function DocsPage() {
  ...
  return (
    <PageShell title="Hifadhi yangu" subtitle="Rufaa, vidonge, na ripoti — zote mahali pamoja, zinazoshirikiwa kwa ridhaa." back={{ to: '/mimi' }}>
      <Card jewel={JEWEL.tealRoho}>
        <Pill tone="teal">Pakia nyaraka mpya</Pill>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 10, marginTop: 14 }}>
          <input placeholder="Kichwa cha nyaraka" value={draft.title || ''} onChange={(e) => setDraft({ ...draft, title: e.target.value })} aria-label="Kichwa" style={inp} />
          <select value={draft.kind} onChange={(e) => setDraft({ ...draft, kind: e.target.value as DocItem['kind'] })} aria-label="Aina ya nyaraka" style={inp}>
          ...
          <label style={{ ...inp, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}>
            {draft.dataUrl ? '✓ Faili tayari' : 'Chagua faili'}
            <input type="file" accept="image/*,application/pdf" onChange={onFile} style={{ display: 'none' }} aria-label="Pakia faili" />
          </label>
      ...
      <h2 style={{ fontFamily: TYPE.serif, fontSize: 26, letterSpacing: TYPE.tighterTrack, margin: '28px 0 14px' }}>Nyaraka zangu</h2>
      {docs.length === 0 && <p style={{ color: TEXT.muted }}>Bado hakuna nyaraka.</p>}
        ...
        {docs.map((d) => (
          <Card key={d.id} jewel={JEWEL.indigoWisdom}>
            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
              ...
              <button onClick={() => { deleteDoc(d.id); setDocs(listDocs()) }} aria-label={`Futa ${d.title}`} style={{ background: 'transparent', border: 'none', color: JEWEL.maroonCrisis, cursor: 'pointer', fontSize: 18 }}>×</button>
            </div>
            <div style={{ display: 'flex', gap: 8, marginTop: 10 }}>
              <a href={d.dataUrl} download={d.title} style={{ padding: '8px 14px', borderRadius: RADII.chip, background: 'rgba(250,245,229,0.85)', border: '1px solid rgba(11,9,8,0.10)', color: TEXT.body, textDecoration: 'none', fontSize: 12 }}>↓ Pakua</a>
              <a href={d.dataUrl} target="_blank" rel="noreferrer" style={{ padding: '8px 14px', borderRadius: RADII.chip, background: 'rgba(250,245,229,0.85)', border: '1px solid rgba(11,9,8,0.10)', color: TEXT.body, textDecoration: 'none', fontSize: 12 }}>↗ Fungua</a>
            </div>
            {circle.length > 0 && (
              <>
                <p style={{ margin: '14px 0 6px', fontSize: 12, color: TEXT.muted }}>Shiriki na:</p>
                {circle.map((m) => (
                  <label key={m.id} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '4px 0', fontSize: 13 }}>
                    <input type="checkbox" checked={d.sharedWith.includes(m.id)} onChange={() => toggleShare(d.id, m.id)} aria-label={`Shiriki na ${m.name}`} />
