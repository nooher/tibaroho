...
export default function Step6Mapendeleo({ profile, update, next, back }: Props) {
  ...
  return (
    <div style={cardStyle}>
      <h2 style={titleStyle}>Mapendeleo</h2>
      <p style={subStyle}>Mwenza ataenda kwa sauti unayopendelea.</p>

      <div style={{ display: 'grid', gap: 16 }}>
        <div>
          <label style={labelStyle}>Sauti ya Mwenza</label>
          <div style={{ display: 'grid', gap: 8 }}>
            {TONES.map((t) => (
              <button key={t.v} type="button" onClick={() => setTone(t.v)} style={{
                ...chipStyle(tone === t.v), textAlign: 'left', padding: '12px 16px', borderRadius: 14,
              }}>
        ...
        <div>
          <label style={labelStyle}>Imani (chaguo, sio lazima)</label>
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            {FAITHS.map((f) => <button key={f.v} type="button" onClick={() => setFaith(f.v)} style={chipStyle(faith === f.v)}>{f.label}</button>)}
          </div>
        </div>

        <div>
          <label style={labelStyle}>Duara la usalama (hadi watu 3 wa kuaminika)</label>
          {trust.map((c, i) => (
            <div key={i} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8, marginBottom: 8 }}>
              <input style={inputStyle} placeholder="Jina" value={c.name} onChange={(e) => updateTrust(i, 'name', e.target.value)} aria-label="Jina la mtu wa kuaminika" />
              <input style={inputStyle} placeholder="Uhusiano" value={c.relation} onChange={(e) => updateTrust(i, 'relation', e.target.value)} aria-label="Uhusiano" />
              <input style={inputStyle} placeholder="+255…" value={c.phone} onChange={(e) => updateTrust(i, 'phone', e.target.value)} aria-label="Simu" />
            </div>
          ))}
          {trust.length < 3 && (
        ...
        <div>
          <label style={labelStyle}>Njia ya mawasiliano</label>
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            {CONTACT.map((c) => <button key={c.v} type="button" onClick={() => setContact(c.v)} style={chipStyle(contact === c.v)}>{c.label}</button>)}
          </div>
        </div>
      </div>
      ...
      <div style={{ marginTop: 10, fontSize: 11, color: TEXT.hint }}>
        Mawasiliano yote yanabaki Tanzania. Hatuuzi data.
      </div>
    </div>
  );
}
